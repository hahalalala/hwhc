...
func GetAmountIncrRecordId(xmysql  *mysql.XMySQL,userId int64,dateStr string) (id int64) {

	sqlstr := "SELECT id FROM amount_incr_record WHERE user_id = ? and create_date = ?"
	row :=xmysql.QueryRow(sqlstr,userId,dateStr)
	_ = row.Scan(&id)
	return
}